...
<?php 
/* Les pages Chapeaux sont normalement visibles par tout le monde
 * placer les exceptions dans le code ci-dessous
 */ 
if (($node->field_chapeau_nompage[0]['value'] == 'Espace collaboratif') and
            !(user_is_logged_in())) {
    //print "<pre>"; print "Custom Page Chapeau access denied"; print "</pre>";
...
<div class="page-header"> 
	<h1>
		...
		<?php if( $node->title <> $node->field_chapeau_soustitre[0]['value']): ?>
		...
		<span class="page-soustitre-bleu"> 
			<?php print $node->field_chapeau_soustitre[0]['value'];?>
		</span>
	</h1>

	<!--ici la description dans la même classe que la description de la view Top news-->
	<?php if($node->field_chapeau_description[0]['value']): ?>
	<h2>
		<span class="description-orange">
				<?php print $node->field_chapeau_description[0]['value'];?>
		</span>
	</h2>
	<?php endif; ?>
    ...
    <div class="page-content">
        ...
        <div class="texte-gris"> 
            <?php
			//le node concerné
			//$node = node_load($nid); //INUTILE
			//on cherche le texte pour le node concerné
			$texte = $node->field_chapeau_texte[0]['value'];
			if ($texte): 
        ...
        <!--ici la vidéo-->
        <span class="node-presentation-video"> 
            <?php
			//le node concerné
			//$node = node_load($nid);
			//ici le site internet dailymotion
			$site = 'http://www.dailymotion.com/swf';
			$src = $site.'/'.$node->field_chapeau_video[0][value];
			
			if($node->field_chapeau_video[0][value]){
